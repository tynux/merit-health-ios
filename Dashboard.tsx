...
export default function Dashboard() {
  ...
  const settleMeritMutation = trpc.merit.settleDailyMerit.useMutation({
    onSuccess: async () => {
      // Refresh all related queries after settlement
      await utils.merit.getUserStats.invalidate();
      await utils.merit.getDailyRecord.invalidate();
      await utils.health.getDailyData.invalidate();
    },
  });
  ...
  const handleSimulateData = async () => {
    try {
      ...
      // After simulating, settle the merit (onSuccess will invalidate queries)
      await settleMeritMutation.mutateAsync(todayDate);
      // Also refresh health data immediately after simulation
      await utils.health.getDailyData.invalidate();
    } catch (error) {
